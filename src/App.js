...
export default function App() {
  ...
  const setDespesas = (val) => {
    ...
    setDespesasState(next);
    saveStorage("vfc_despesas", next);
  };
        ...
        {aba === "dashboard" && (
          <div>
            ...
            <div className="card" style={{ marginBottom: 20 }}>
              <h3 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 20, fontWeight: 900, marginBottom: 16, color: "#ff4757" }}>⚠️ INADIMPLENTES — {mesFiltro} ({inadimplentes.length})</h3>
                  ...
                  <div key={j.id} className="table-row" style={{ gridTemplateColumns: "2fr 1.5fr 1.5fr 100px 120px" + (isAdmin ? " 180px" : ""), gap: 8, borderBottom: "1px solid #1a2540" }}>
                    ...
                    {isAdmin && (
                      <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                        <button className={`btn ${pag?.pago ? "btn-gray" : "btn-green"}`} style={{ fontSize: 11, padding: "5px 10px" }} onClick={() => togglePagamento(j.id)}>{pag?.pago ? "✅ Pago" : "Marcar Pago"}</button>
                    ...
                    {!isAdmin && pag && <span className={`tag ${pag.pago ? "tag-green" : "tag-red"}`}>{pag.pago ? "Pago" : "Pendente"}</span>}
                  </div>
                );
              })}
              {jogadores.length === 0 && (
